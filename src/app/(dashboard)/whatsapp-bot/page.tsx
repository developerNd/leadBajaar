      ...
      <div className="h-full flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-slate-950/20">
        <Card className="max-w-md w-full border-dashed border-2 border-slate-200 dark:border-slate-800 shadow-none bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-3xl overflow-hidden">
          <CardHeader className="text-center pb-2">
            <div className="h-20 w-20 bg-primary/10 dark:bg-primary/10 rounded-3xl flex items-center justify-center mx-auto mb-6">
              <Zap className="h-10 w-10 text-primary dark:text-indigo-400" />
            </div>
          ...
          <CardFooter className="pt-2 pb-8 px-8">
            <Link href="/billing" className="w-full">
              <Button className="w-full h-12 bg-primary hover:bg-primary/90 text-white font-black rounded-2xl shadow-xl shadow-primary/20 transition-all text-sm uppercase tracking-widest">
                Upgrade to Pro
              </Button>
            </Link>
          </CardFooter>
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <div className="flex-1 min-h-0 flex flex-col">
        <Card className="flex-1 flex flex-col shadow-none border-0 bg-transparent rounded-none overflow-hidden">
            ...
            <CardHeader className="shrink-0 px-2 sm:px-6 pb-2 sm:pb-4 border-b border-slate-100 dark:border-slate-800/50">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <TabsList className="bg-transparent h-12 p-0 gap-6 sm:gap-8 justify-start overflow-x-auto no-scrollbar">
                  <TabsTrigger 
                    value="active" 
                    className="h-full rounded-none border-b-2 border-transparent bg-transparent px-1 pb-4 pt-4 font-black text-[11px] uppercase tracking-widest text-slate-500 data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:text-slate-900 dark:data-[state=active]:text-white data-[state=active]:shadow-none transition-all hover:text-slate-700 dark:hover:text-slate-300"
                  >
                    Flows
                  </TabsTrigger>
                  <TabsTrigger 
                ...
                <div className="flex items-center gap-2">
                    ...
                    {sessions.map(id => (
                      <div 
                        ...
                        className={cn(
                          ...
                          selectedUser === id ? "border-primary ring-1 ring-indigo-500" : "border-emerald-500/20"
                        )}
                      >
                    ...
                    {ghostSessions.map(id => (
                      <div 
                        ...
                        className={cn(
                          ...
                          selectedUser === id ? "border-primary ring-1 ring-indigo-500" : "border-slate-200 dark:border-slate-800"
                        )}
                      >
                        ...
                        <Button 
                          variant="ghost" size="icon" className="h-5 w-5 text-indigo-400 hover:text-primary hover:bg-primary/10 rounded-md ml-1"
                  ...
                  {sessions.length === 0 && (
                    <Button 
                      ...
                      className="h-9 px-4 rounded-xl border-primary/30 text-primary dark:text-indigo-400 hover:bg-primary/10 dark:hover:bg-primary/10 transition-all font-black text-[10px] uppercase tracking-widest"
                    >
                      ...
                      {!selectedUser ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 dark:bg-slate-950/20 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                          ...
                          <Link href={`/whatsapp-bot/builder?userId=${selectedUser}`} className="mt-6">
                            <Button className="bg-primary hover:bg-primary/90 text-white font-black px-8 rounded-xl shadow-lg shadow-primary/20 transition-all">Start Building</Button>
                          </Link>
                        </div>
                      ) : (
                          ...
                          <Card key={flow.id} className={cn(
                            ...
                            <div className="p-4 sm:p-5 flex flex-col sm:flex-row items-start gap-4">
                              <div className={cn(
                                ...
                                flow.is_active ? "bg-primary/10 dark:bg-primary/10 text-primary dark:text-indigo-400" : "bg-slate-100 dark:bg-slate-800 text-slate-400"
                              )}>
                              ...
                              <div className="flex-1 min-w-0 space-y-3">
                                <div className="flex items-center justify-between gap-3">
                                  ...
                                  <div className="flex items-center gap-1 shrink-0">
                                    <Link href={`/whatsapp-bot/builder?userId=${selectedUser}`}>
                                      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-slate-400 hover:text-primary hover:bg-primary/10 dark:hover:bg-primary/10">
                                ...
                                <div className="flex flex-col space-y-2">
                                  <div className="flex items-center gap-2">
                                    <Badge className="bg-primary/10 text-primary hover:bg-primary/20 border-none text-[9px] font-black px-2 py-0.5 shrink-0">
