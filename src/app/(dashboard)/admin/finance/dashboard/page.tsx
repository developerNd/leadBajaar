    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Financial overview for the selected period</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {/* Revenue */}
        <Card className="border-emerald-200/50 bg-gradient-to-br from-emerald-50/50 dark:from-emerald-950/20">
          <CardContent className="pt-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">{highlights?.active_workspaces ?? 0} active workspaces</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-red-200/50 bg-gradient-to-br from-red-50/50 dark:from-red-950/20">
          <CardContent className="pt-5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Total Burn</span>
            ...
            <p className="text-xs text-muted-foreground mt-1">Ops {fmt(expenses?.operational ?? 0)} + Payroll {fmt(expenses?.payroll ?? 0)}</p>
          </CardContent>
        </Card>

        {/* Net P&L */}
        <Card className={`border-2 ${pnl?.is_profitable ? 'border-emerald-200/60 bg-gradient-to-br from-emerald-50/30' : 'border-red-200/60 bg-gradient-to-br from-red-50/30'} dark:from-transparent`}>
          <CardContent className="pt-5">
            <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">Margin {pnl?.gross_margin ?? 0}%</p>
          </CardContent>
        </Card>

        {/* Payroll Status */}
        <Card className="border-indigo-200/50 bg-gradient-to-br from-indigo-50/50 dark:from-indigo-950/20">
          <CardContent className="pt-5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Payroll</span>
              <div className="h-7 w-7 rounded-lg bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center">
                <Users className="h-3.5 w-3.5 text-indigo-600" />
              </div>
            </div>
            <p className="text-2xl font-bold text-indigo-600">{fmt(payroll?.total ?? 0)}</p>
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent>
            <div className="space-y-1 text-sm font-mono">
              <div className="flex justify-between py-1.5 border-b font-semibold text-foreground/70 text-xs uppercase tracking-wider">
              ...
              {revenue?.adjustments !== 0 && (
                <div className="flex justify-between py-1 pl-4">
                  <span className="text-muted-foreground">Adjustments & Fees</span>
              ...
              <div className="flex justify-between py-1.5 border-b font-semibold text-foreground/70 text-xs uppercase tracking-wider mt-2">
              ...
              <div className="flex justify-between py-1 pl-4">
                <span className="text-muted-foreground">Salaries & Payroll</span>
              ...
              <div className={`flex justify-between py-2 px-2 rounded-lg mt-2 font-bold text-base ${pnl?.is_profitable ? 'bg-emerald-50 dark:bg-emerald-950/30' : 'bg-red-50 dark:bg-red-950/30'}`}>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <Card className="bg-slate-900 text-white border-none overflow-hidden relative">
              ...
              <CardContent className="pt-5">
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Cash Flow Projection</p>
                <p className="text-xl font-bold mt-2 text-emerald-400">Stable & Growing</p>
                <p className="text-[10px] text-slate-400 mt-2">Retention rate is healthy. Net margin trending up.</p>
              </CardContent>
            </Card>
          </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-12">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase">Burn</span>
                ...
                {expenses.by_category.map((cat: any, i: number) => (
                  <div key={cat.category} className="flex items-center justify-between text-sm group">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-muted-foreground group-hover:text-foreground transition-colors">{cat.category}</span>
                    </div>
                    <div className="text-right">
                      <p className="font-bold">{fmt(cat.total)}</p>
                      <p className="text-[10px] text-muted-foreground">{((cat.total / expenses.total) * 100).toFixed(1)}%</p>
                    </div>
                  </div>
                ))}
              </div>
